...
- Why aren't my snap commands working, specifically for CMake?
  ...
  - If snap still is not working, you can also install CMake with this command: <br />
 `sudo apt-get -y install cmake --fix-missing` ([source](https://stackoverflow.com/questions/74059441/how-can-i-install-a-specific-version-of-cmake-with-wsl-on-windows))
    - You might get a weird "failed to fetch ..." message, but everything is good if <br />`cmake --version` works
  - **Note:** the linked article also has directions for building CMake from source. This option will work if all others fail but is quite painful.
  - Feel free to reach out if you are having issues installing CMake or anything else.
